...
for i in `cat $HOURS | awk -F'|' '{print $2}' | sort -n | uniq`
do
	...
	echo >> $MAIL
       ...
       /bin/mail -s "[$DATE] KPIONLINE - SLA due in 24 hours for $i" -a $MAIL "dcalves@br.ibm.com,ieaubr4@br.ibm.com,jbpiton@br.ibm.com,$i" < $MSG
done
